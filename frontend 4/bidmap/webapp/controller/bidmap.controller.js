	sap.ui
	.define([ "com/southend/bidmap/controller/BaseController",
			'sap/ui/model/json/JSONModel',
			"com/southend/bidmap/controller/formatter",
			"sap/m/MessageBox", "sap/ui/core/Fragment", "sap/m/Dialog",
			"sap/m/Label", "sap/ui/export/Spreadsheet", "sap/m/MessageToast","sap/m/Text","sap/m/MessagePopover","sap/m/MessageItem", ], function(BaseController, JSONModel, formatter, MessageBox, Fragment, Dialog, Label, Spreadsheet,MessageToast,Text,MessagePopover,MessageItem) {
		"use strict";
		var oMessagePopover;
		return BaseController
				.extend("com.southend.bidmap.controller.bidmap", {
					formatter: formatter,
					onInit: function () {
						//this.getRouter().getRoute("Targetmapa").attachPatternMatched(this.onRouteMatched, this);
						...
						this._oControlModel = new sap.ui.model.json.JSONModel(
							this._oControlModelData);
						this.getView()
							.setModel(this._oControlModel, "Control");
						//
					...
					onUploadComplete: function(){
						this.getModel("mcMapHelpers").refresh();
					...
					LoadRFPData: function (sDocument) {
						this
							.getView()
							.getModel("mcdetails")
							.attachRequestFailed(this.onmcdetailsModelLoadFailed
								.bind(this));
						this
							.getView()
							.getModel("mcdetails")
							.attachRequestSent(this.onmcdetailsModelLoadSent
								.bind(this));
						this
							.getView()
							.getModel("mcdetails")
							.attachRequestCompleted(this.onmcdetailsModelLoadCompleted
								.bind(this));
						this.sDocument = sDocument;
						// this.buildHTMLTable();
						if(this.sDocument != null){
							this.getView().getModel("mcdetails").loadData("http://eccdev:443/sap/bc/zariba/bidmap/?docId="+ sDocument );
						}else{
							this.getView().getModel("mcdetails").loadData("./model/MapaComparativo.json");
						}
					},
					HeaderChangeDocument: function (oEvt) {
						var dialog = new Dialog({
							...
							beginButton: new sap.m.Button({
								...
								enabled: false,
								press: function () {
									var sDocument = sap.ui
										.getCore()
										.byId('submitDialogTextarea')
										.getValue();
									this
										.LoadRFPData(sDocument);
									dialog.close();
								}.bind(this)
							}),
					...
					onmcdetailsModelLoadCompleted: function (oEvent) {
						...
						if (!oEvent.getParameter("success")) {
							MessageBox
								.error(oEvent
									.getParameter("errorobject").responseText, {
					...
					/*ValorUltimoPrecoCorrigido: function (aSuppliers, nIndiceIGPM) {
						if (!aSuppliers) {
							return "";
						}
						if (nIndiceIGPM === 0 || !nIndiceIGPM) {
							return this.FormatCurrency((nLastPOPricesSum)
								.toFixed(2));
						}
						var nSum = 0;
						for (var i = 0; i < aSuppliers.length; i++) {
							nSum += this
								.ValorCorrigidoUltPrecoSomatorio(aSuppliers[i].lastpopricesumquoteaward, this.aSuppliers[i].lastpopricesumquoteawardavg,
									nIndiceIGPM);
						}
						return this.FormatCurrency(nSum.toFixed(2));
					},*/
					...
					MessagesShow: function (oEvent) {
						if (!this._oDialog) {
							this._oDialog = sap.ui
								.xmlfragment("fragmentMapaLogMensagens", "com.southend.bidmap.view.Mapa.MapaLogMensagens", this);
							this.getView().addDependent(this._oDialog);
							this._oDialog.setModel(this.getView()
								.getModel("mcdetails"), "mcdetails");
						}
						/*this._oDialog.bindElement("mcdetails>" + oEvent.getSource().getParent()
